...
def bfs(graph, root, max_depth):
	...
	"""
	###TODO
	#node2distances,node2num_paths,node2parents = dict()
	node2distances = dict()
	node2num_paths = dict()
	node2parents = dict()
	count = 0
	...
	node2num_paths[root]=count+1
	NodeSeen = [root]
	while visited:
		NodePop = visited.popleft()
		neighbor= sorted(graph.neighbors(NodePop))
		count = node2distances[NodePop]
		if count >= max_depth:
		    break;
		for node in neighbor:
			if node in NodeSeen:
				LastCount = node2distances[node]
				CurrentCount = count + 1
				if(LastCount==CurrentCount):
				    node2num_paths[node] += node2num_paths[NodePop]
				    node2parents[node].append(NodePop)
			else:
				NodeSeen.append(node)
				visited.append(node)
				node2parents[node] = [NodePop]
				node2distances[node] = count + 1
				node2num_paths[node] = node2num_paths[NodePop] 
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    ###TODO
    NodeCount = defaultdict(lambda:1)
    EdgeCredit = {}
    ...
    for i in reversed(list(node2distances)):
      ...
      for j in node2parents[i]:
        if i<j:            
          EdgeCredit[(i,j)] = float(NodeCount[i]/num_parents)          
          NodeCount[j]+=EdgeCredit[(i,j)]
        else:
          EdgeCredit[(j,i)] = float(NodeCount[i]/num_parents)          
          NodeCount[j]+=EdgeCredit[(j,i)]                              
...
def partition_girvan_newman(graph, max_depth):
	...
	betweenness = sorted(sorted(approximate_betweenness(CopyOfGraph, max_depth).items()), key=lambda x: (-x[1],x[0]))
	components = get_components(CopyOfGraph)
	while (True):
	    CopyOfGraph.remove_edge(*(betweenness[i][0]))
	    components = get_components(CopyOfGraph)
	    if len(components) > 1:
	        break
...
def get_subgraph(graph, min_degree):
	...
	for node in graph.nodes():
	    if graph.degree(node)>= min_degree:
	        OutputSubGraph.add(node)
	return graph.subgraph(OutputSubGraph)
	pass
...
def volume(nodes, graph):
    ...
    for edge in graph.edges():
        if(((edge[0] in nodes) or (edge[1] in nodes)) and (edge not in visited)):
                visited.append(edge)
    return len(visited)
    pass
...
def norm_cut(S, T, graph):
    ...
    return ((float(CutGraph) / volume(T, graph)) + (float(CutGraph) / volume(S, graph)))
    pass
...
def score_max_depths(graph, max_depths):
	...
	for depth in max_depths:
	    ...
	    if(len(components)>1):
	        ...
	        ListOfMaximumCut.append((depth,NormalCut))
	return ListOfMaximumCut
	pass
...
def make_training_graph(graph, test_node, n):
    ...
    """
    ###TODO
    TemporaryGraph=graph.copy()
    NodeList=sorted(TemporaryGraph.neighbors(test_node))
    for item in NodeList[0:n]:
            TemporaryGraph.remove_edge(*(test_node,item))    
...
def jaccard(graph, node, k):
    ...
    for nodeitem in graph.nodes():
        if nodeitem != node and not graph.has_edge(node, nodeitem):
            ...
            EdgesList.append(((node,nodeitem),jaccard))
    return sorted(EdgesList,key=lambda x: -x[1])[0:k]
    pass
...
def evaluate(predicted_edges, graph):
	...
	"""
	###TODO
	FractionOfPredictedEdges=0
	for edge in range(len(predicted_edges)):
	    if(graph.has_edge(*(predicted_edges[edge]))):
	        FractionOfPredictedEdges += 1
	return float(FractionOfPredictedEdges/len(predicted_edges))
	pass
